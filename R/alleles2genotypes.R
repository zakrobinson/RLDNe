...
#'@param name_pattern The locus-allele identifiers in column names specified by regex capture group (e.g., "(.+)_(A.$) performs the same as name_sep="_"). This is useful when the locus-allele separator string is repeated in column names (e.g, Loc_1_A1). This argument takes precedent over name_sep.
#'@param suffix Logical that allele identifier is a suffix (e.g., Loc1_A1 instead of A1_Loc1). Default=T.
#'@param Adelim Delimiter string between alleles. The default="" is not ideal when allele character lengths vary.
