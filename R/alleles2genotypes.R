...
#'@param allelesAsIntegers For microsats with leading zeros use FALSE. If FALSE this will eliminate the loss of leading zeros, but will not be usable in 'hierfstat'. When TRUE the genotypes are converted to integers for 'hierfstat' package.
#'@author Zak Robinson, Contact: zachary.robinson(at)umontana.com
#'@return Data frame in genotype per column format
#'@examples
#' data("wgp_example")
#' alleles2genotypes(wgp_example_2col,allele_cols = 3:ncol(wgp_example_2col),allelesAsIntegers = T)
#'@export




alleles2genotypes<-function(df,allele_cols,allelesAsIntegers=T){
...
###
  ...
  group<-matrix(data=allele_cols,byrow=T,ncol=2)
  ...
  out_df<-as.data.frame(matrix(ncol=(ncol(loci_mat)+length(meta_data_cols)),nrow = nrow(loci_mat)),stringsAsFactors=F)
