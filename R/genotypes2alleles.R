...
#'@param fixed_A The number of characters of alleles. A diploid genotype "AT" has a fixed_A=1. Only used when the delimiter between alleles is "".
#'@param delim A non-zero length character string separating alleles. For example, ":" for "A:T" or "-" 114-116"
...
genotypes2alleles <-function(df,loci,fixed_A=NULL,delim=NULL){
  ...
  if(!is.null(delim)){
    NAtest <- df %>% select(loci) %>% reframe(across(everything(),~sum(grepl(delim,.x))/sum(!is.na(.x))))
    ...
    alleles_df <-  df %>%
      ...
      tidyr::pivot_longer(cols = c(A1,A2),
                          names_to = "allele") %>%
      tidyr::pivot_wider(names_from = c(locus,allele),
                         values_from = value,names_sort = F)
  }
